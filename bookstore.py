...
def register():
    ...
    try:
        cursor.execute(
            """
            INSERT INTO Members (email, password, fname, lname, address, city, zip, phone)
...
def login():
    ...
    cursor.execute("SELECT userid, fname, lname, email FROM Members WHERE email = %s AND password = %s", (email, password))
    member = cursor.fetchone()

    if member:
        userid = member[0]  # Extract the userid from the first column of the result
        print(f"Welcome back, {member[1]} {member[2]}!")  # Print user's full name (first name, last name)
